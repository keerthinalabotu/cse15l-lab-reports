...
The command that I took to find alternatives for was `less` which basically allows us to view the content of a file one at a time on a screen so that we can then quit out of it using `q`. 

An example of using the `less` command is this: 
The commands would start off like this: 

```
keert@keerthi-n MINGW64 ~/docsearch (main)
$ bash count-txts.sh technical/plos
 252  252 8880 grep-results.txt

keert@keerthi-n MINGW64 ~/docsearch (main)
$ less find-results.txt
```
The output of the `less` command above would be this: 
```
technical/plos
...
The above shows the files within the `technical/plos` directory on a separate screen and afterward after I type `q` the output dissapears and the request for a new bash command shows up.

**First alternative**
The first alternate command would be `cat` which also displays the contents of the directory given to it, although it doesn't necessarily show it in a separate screen or anything. 
...
The output of using the `cat` command is the following: 
...
Another example of using `cat` would be the following:
```
keert@keerthi-n MINGW64 ~/docsearch (main)
$ bash count-txts.sh technical/biomed
  837   837 29816 grep-results.txt

keert@keerthi-n MINGW64 ~/docsearch (main)
$ cat find-results.txt
```
The output of the input above would be the following:
```
technical/biomed
